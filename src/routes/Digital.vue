<template>
  <main class="pt-16">
    <section class="container mx-auto px-4 2xl:px-20 w-full mt-10 mb-20">

      <div class="text-sm breadcrumbs">
        <ul>
          <li>홈</li> 
          <li>디지털</li>
        </ul>
      </div>
      
      <section class="pt-12 pb-8 container mx-auto">
        <h2 class="pb-8 text-4xl font-bold text-center">디지털</h2>
        <div class="grid grid-cols-4 w-full gap-6">
          <!-- 상품 v-for -->
            <div 
              class="card card-compact bg-gray-100 grid flex-grow dark:bg-gray-700 cursor-pointer border-solid border border-gray-200 dark:border-none"
              v-for="{ id, title, price, image } in digi" :key="id"
            >
              <RouterLink :to="'/product/' + id">
                <figure class="bg-white overflow-hidden h-80"><img :src="image" class="object-contain h-1/2 w-1/2" /></figure>
                <div class="card-body h-44">
                  <h2 class="card-title text-xs xl:text-base">{{ title }}</h2>
                  <p class="xl:text-base">${{ price }}</p>
                </div>
              </RouterLink>
            </div>
        </div>
      </section>

    </section>
  </main>
</template>

<script>
import { mapState } from 'vuex'

export default {
  computed: {
    ...mapState('goods', [
      'digi'
    ])
  }
}
</script>